...
MS_NAME: str = 'cw-prospect-ms'  # NOMBRE DEL MS, CAMBIAR CON EL NOMBRE DE CADA MS
LOG_SIZE: int = 10          # CANTIDAD EN MB Ejemplo: (20 = 20MB), debe ser mayor a 1, número entero sin decimales
...
def start_request(channel: str, system: str, function_name, internal_process, log: bool = True):
    if log:
        try:
            internal_transaction_id, external_transaction_id = internal_process
...
def execute_duration(start_time, function_name, internal_process, log: bool = True):
    if log:
        try:
            internal_transaction_id, external_transaction_id = internal_process
